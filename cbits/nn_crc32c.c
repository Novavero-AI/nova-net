...
/* ---------------------------------------------------------------------------
 * Software fallback — byte-at-a-time table lookup
 *
...
#if defined(NN_CRC32C_SW) || defined(NN_CRC32C_TEST_SW)
...
#endif /* NN_CRC32C_SW || NN_CRC32C_TEST_SW */
...
/* ---------------------------------------------------------------------------
 * Test-only: software CRC32C accessible regardless of hardware support.
 * Compiled when NN_CRC32C_TEST_SW is defined.
 * ------------------------------------------------------------------------- */

#ifdef NN_CRC32C_TEST_SW

uint32_t
nn_crc32c_software_test(const uint8_t *buf, size_t len)
{
    return crc32c_software(0, buf, len);
}
